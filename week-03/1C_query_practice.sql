...
ORDER BY 
    UnitPrice ASC;
-- Returns the product prices in ascending order
...
ORDER BY
	UnitPrice DESC;
-- The products that we carry that have atleast 100 units on hand
...
ORDER BY
	UnitPrice DESC,
    ProductName ASC;
-- Ordered by unit price and product name
...
FROM
	northwind.orders;
-- Returns CustomerID as Customer Count
...
SELECT
	ShipCountry, COUNT(DISTINCT CustomerID) AS CustomerCount
FROM
	northwind.orders
GROUP BY
	ShipCountry
